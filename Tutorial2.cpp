...
const wchar_t* Tutorial2::c_hitGroupName = L"MyHitGroup";
const wchar_t* Tutorial2::c_raygenShaderName = L"MyRaygenShader";
const wchar_t* Tutorial2::c_closestHitShaderName = L"MyClosestHitShader";
const wchar_t* Tutorial2::c_missShaderName = L"MyMissShader";
...
void Tutorial2::SerializeAndCreateRaytracingRootSignature(D3D12_ROOT_SIGNATURE_DESC& desc, ComPtr<ID3D12RootSignature>* rootSig)
{
	auto device = Application::Get().GetDevice();
...
void Tutorial2::CreateRaytracingRootSignatures()
{
...
void Tutorial2::CreateRaytracingDeviceDependentResources()
{
	//auto device = Application::Get().GetDevice();
...
void Tutorial2::BuildTriangle(ID3D12GraphicsCommandList* commandList)
{
...
void Tutorial2::BuildRenderTriangleItem()
{
...
void Tutorial2::ReleaseWindowSizeDependentResource()
{
...
void Tutorial2::ReleaseDeviceDependentResource()
{
...
void Tutorial2::CreateRaytracingWindowSizeDependentResources()
{
	CreateRaytracingOutputResource();
...
void Tutorial2::CreateRaytracingOutputResource()
{
	auto device = Application::Get().GetDevice();
...
void Tutorial2::BuildShaderTables()
{
	auto device = Application::Get().GetDevice();
...
void Tutorial2::CreateRaytracingDeviceCommandlist()
{
	auto device = Application::Get().GetDevice();
	auto commandQueue = Application::Get().GetCommandQueue(D3D12_COMMAND_LIST_TYPE_DIRECT);
	auto commandList = commandQueue->GetCommandList();
...
void Tutorial2::CreateRaytracingPipelineStateObject()
{
...
// Local root signature and shader association
// This is a root signature that enables a shader to have unique arguments that come from shader tables.
void Tutorial2::CreateLocalRootSignatureSubobjects(CD3D12_STATE_OBJECT_DESC* raytracingPipeline)
{
...
void Tutorial2::BuildAccelerationStructures()
{
	auto device = Application::Get().GetDevice();
	auto commandQueue = Application::Get().GetCommandQueue(D3D12_COMMAND_LIST_TYPE_DIRECT);
	auto commandList = commandQueue->GetCommandList();
...
void Tutorial2::CopyRaytracingOutputToBackbuffer(ID3D12GraphicsCommandList* commandList)
{
...
WRAPPED_GPU_POINTER Tutorial2::CreateFallbackWrappedPointer(ID3D12Resource* resource, UINT bufferNumElements)
{
	auto device = Application::Get().GetDevice();
...
UINT Tutorial2::AllocateDescriptor(D3D12_CPU_DESCRIPTOR_HANDLE* cpuDescriptor, UINT descriptorIndexToUse)
{
...
void Tutorial2::DoRaytracing(ID3D12GraphicsCommandList* commandList)
{
...
void Tutorial2::UpdateForSizeChange(UINT width, UINT height)
{
	float border = 0.01f;
