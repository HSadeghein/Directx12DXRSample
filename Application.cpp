...
using WindowsMap = std::map<HWND, WindowPtr>;
using WindowNameMap = std::map<std::wstring, WindowPtr>;

static Application* gs_pSingelton = nullptr;
static WindowsMap gs_Windows;
...
Application::Application(HINSTANCE hInst) : g_hInstance(hInst),g_TearingSupported(false)
{
	...
	wndClass.hIcon = LoadIcon(g_hInstance, nullptr);
	wndClass.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wndClass.lpszMenuName = nullptr;
	wndClass.lpszClassName = WINDOW_CLASS_NAME;
	wndClass.hIconSm = LoadIcon(g_hInstance, nullptr);
	...
	if (g_d3d12Device)
	{
		g_DirectCommandQueue = std::make_shared<CommandQueue>(g_d3d12Device, D3D12_COMMAND_LIST_TYPE_DIRECT);
		g_ComputeCommandQueue = std::make_shared<CommandQueue>(g_d3d12Device, D3D12_COMMAND_LIST_TYPE_COMPUTE);
		g_CopyCommandQueue = std::make_shared<CommandQueue>(g_d3d12Device, D3D12_COMMAND_LIST_TYPE_COPY);
...
void Application::Destroy()
{
	if (gs_pSingelton)
	{
		assert(gs_Windows.empty() && gs_WindowByName.empty() && "All windows should be destroyed before destroying th application.");
...
std::shared_ptr<Window> Application::CreateRenderWindow(std::wstring & windowName, int clientWidth, int clientHeight, bool vSync)
{
	WindowNameMap::iterator it = gs_WindowByName.find(windowName);
...
{
	WindowPtr windowPtr = GetWindowByName(windowName);
...
std::shared_ptr<CommandQueue> Application::GetCommandQueue(D3D12_COMMAND_LIST_TYPE type) const
{
	...
	case D3D12_COMMAND_LIST_TYPE_DIRECT:
		commandQueue = g_DirectCommandQueue;
		break;
...
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> Application::CreateDescriptorHeap(UINT numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type)
{
	...
	ThrowifFailed(g_d3d12Device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&descriptorHeap)));
...
{
	return g_d3d12Device->GetDescriptorHandleIncrementSize(type);
}

static void RemoveWindow(HWND hWnd)
{
	WindowsMap::iterator it = gs_Windows.find(hWnd);
	if (it != gs_Windows.end())
	{
		WindowPtr windowPtr = it->second;
		gs_WindowByName.erase(windowPtr->GetWindowName());
		//gs_Windows.erase(it);
		gs_Windows.erase(windowPtr->GetWindowHandle());
	}
}
